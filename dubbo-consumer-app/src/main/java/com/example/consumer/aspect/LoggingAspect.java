...
public class LoggingAspect {
    ...
    // 定义切点，拦截 HelloController 的所有方法
    @Pointcut("execution(* com.example.consumer.controller.HelloController.*(..))")
    public void helloControllerMethods() {
    }

    // 在方法执行前添加统一的 Tag
    @Before("helloControllerMethods()")
    public void beforeMethod(JoinPoint joinPoint) throws InterruptedException {
        // 统一采集访问量指标
        String methodName = joinPoint.getSignature().getName();
        meterRegistry.counter("dubbo_consumer_method_count", "method", methodName).increment();
    ...
    @AfterReturning(pointcut = "helloControllerMethods()", returning = "result")
    ...
    private void addTags(JoinPoint joinPoint) {
        String methodName = joinPoint.getSignature().getName();
        double current = meterRegistry.counter("dubbo_consumer_method_count", "method", methodName).count();
    ...
    // 在方法抛出异常后进行处理
    @AfterThrowing(pointcut = "helloControllerMethods()", throwing = "exception")

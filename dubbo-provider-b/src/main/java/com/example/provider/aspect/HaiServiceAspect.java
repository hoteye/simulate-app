...
public class HaiServiceAspect {
    ...
    private static final Logger logger = LoggerFactory.getLogger(HaiServiceAspect.class);
    ...
    public void beforeMethod(JoinPoint joinPoint) throws InterruptedException {
        ...
        meterRegistry.counter("dubbo_provider_b_method_count", "method", methodName).increment();
    ...
    private void addTags(JoinPoint joinPoint) {
        String methodName = joinPoint.getSignature().getName();
        double current = meterRegistry.counter("dubbo_provider_b_method_count", "method", methodName).count();
